...
start date, end date, graph type  and organisation into _Date_ , _Report.GraphType_ and _String: organisation_ object respectively.
...
The details of execution can be seen from *Figure 11*.

If a valid `report print` command was input, the _PrintReportCommandParser_  will parse the input and convert the Strings :
start date, end date, graph into _Date_ , _Report.GraphType_ and _String: organisation_ object respectively. These objects
...
tart date, end date, graph into _Date_ , _Report.GraphType_ , _String: organisation_ and _String: file name_ object respectively.
