...
the <<Calendar, Calendar>> feature to view any date of the month.
...
application. It matches what you type into the command box and tries to
...
E.g. `exp add -i chicken rice -a 3.5 -a 4.0`. The new expenditure record will have an amount fo 4.0 instead of 3.5.
...
ends at 2 July 2020.
* `repeat edit 3 -ed 2021-07-3"
Edit the end date for the repeated expenditure, this allows user to extend or shorten the whole duration.
...
[[cmd-find]]
=== Search by keyword or tag: `find`

Find expenditures (both single and repeated) which contain the keyword(s) *and* the specified tag (if present). +
...
* `find rice` +
Returns any expenditures or repeats containing `rice`, e.g. `Chicken rice` and `Veg Rice`.
* `find Spicy Chicken Rice` +
Returns any expenditures or repeats that contains *any one* of the keywords `Spicy`, `Chicken`, or `Rice`.
* `find -t Food` +
Returns all expenditures and repeats with the tag `Food`
* `find rice -t Food` +
Returns all expenditures and repeats that contains the keyword `rice` *and* has the tag `Food`.
...
1. Before beginning this section, you can refer to <<Report (Ng Xin Pei), Report>> to look
...
Similarly, keep number of tags to be *within 12* for reports generated by tags. It is possible that not all months or
...
* Exports the report of a graph type populated with expenditure data from
 start date to end date into Report folder with file's name being FILE_NAME.
* `START_DATE` is the date from which the report will start generating from.
* `END_DATE` is the date from which the report will stop generating.
* `GRAPH_TYPE` is the type of the graph you want to generate. Possible values are:
** `bar`: bar graph
...
[NOTE]
The file can be identified by FILE_NAME.png whereby FILE_NAME is specified by your input in the command.
The file can be found in the Report folder which will be in the same directory as the jar file if you are running it by double-clicking the jar file.
If you run the jar file on cmd, the Report folder will be located in the directory where you run the jar file.
...
* <<cmd-find, *find*>> : `find [KEYWORD...] [-t TAG]` +
e.g. `find chicken rice`

* <<setbudget, *setbudget*>> : `setbudget -a AMOUNT [-ym YEAR_MONTH]` +
...
e.g `repeat delete INDEX`
    ...
    Note: PERIOD = daily | weekly | monthly | annually
...
*GRAPH_TYPE: either `pie` or `bar`* +
*ORGANISATION: either `tag` or `month`.

[NOTE]
The window will show "Not Responding" if the default printer is not set up,
this will last for a few seconds. Eventually, a response stating "Set available printer as default printer before printing"
will be given.
