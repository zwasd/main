...
Switch to a different account using the command +
`acc checkout ACCOUNT`: +
image:acc_checkout_CCA.png[]

image:acc_checkout_Home.png[]

Refer to <<Account command, Account Command>> for more details on commands, including renaming, deletion and clearing of data.
...
Refer to <<Expenditure command, Expenditure Command>> for more details on how to add, edit, and delete expenditures.

=== Repeat
_{ work in progress }_

Refer to <<Repeat command, Repeat Command>> for more details.

=== Budget
_{ work in progress }_

Refer to <<Budget command: `setbudget`, Budget Command>> for more details.
...
The _calendar_ feature aims to ease users' navigation experience. +
  *1*. Users can click on the calendar to traverse between different dates. Or, +
  *2*. Users can make use of `go` command to navigate to another date.
...
* `ACCOUNT` is also case-sensitive.
