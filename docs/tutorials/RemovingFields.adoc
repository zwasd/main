...
In this tutorial, we'll do exactly just that and remove the `date` field from `Person`.
...
The `date` field in `Person` is actually an instance of the `seedu.address.model.person.Datelass.
...
. Safe delete the field `date` in `EditPersonDescriptor`
...
. Remove the usages of `date` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `date` field from the `Person` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `date` in your code (`Edit` > `Find` > `Find in path`).
...
For example, there is a `$date` in each `PersonCard` that has not been removed nor identified.

image::$date.png[width=1090px, height=890px]
...
While keeping the `date` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `date` field.
