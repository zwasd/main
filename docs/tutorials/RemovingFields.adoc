...
In this tutorial, we'll do exactly just that and remove the `id` field from `Person`.
...
The `id` field in `Person` is actually an instance of the `seedu.id.model.expenditure.Address` class.
...
. Safe delete the field `id` in `EditPersonDescriptor`
...
. Remove the usages of `id` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `id` field from the `Person` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `id` in your code (`Edit` > `Find` > `Find in path`).
...
For example, there is a `$id` in each `PersonCard` that has not been removed nor identified.

image::$id.png[width=1090px, height=890px]
...
While keeping the `id` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `id` field.
