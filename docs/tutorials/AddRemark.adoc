...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the person's info).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedPerson = new Person(personToEdit.getInfo(), personToEdit.getPhone(), personToEdit.getEmail(),
...
Following convention, let's change the info of the generated method to `execute_addRemarkUnfilteredList_success`.
