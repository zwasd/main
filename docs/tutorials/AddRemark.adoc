...
This command allows users of the AddressBook application to add optional remarks to people in their id book and edit it if required.
...
Let's start by creating a new `RemarkCommand` class in the `src/main/java/seedu/id/logic/command` directory.
...
[source, java]
----
package seedu.id.logic.commands;

import seedu.id.model.Model;

/**
 * Changes the remark of an existing expenditure in the id book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the expenditure identified "
            + "by the index number used in the last expenditure listing. "
...
[source, java]
----
import static seedu.id.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
    ...
    /**
     * @param index of the expenditure in the filtered expenditure list to edit the remark
     * @param remark of the expenditure to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.id.logic.parser` package.
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the expenditure's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a expenditure.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.id.model.expenditure`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each expenditure.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.id.ui.PersonCard`].
...
Not doing so will cause AddressBook to default to an empty id book!
...
.PersonCard.java
----
public PersonCard(Person expenditure, int displayedIndex) {
    //...
    remark.setText(expenditure.getRemark().value);
}
----
