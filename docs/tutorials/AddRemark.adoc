...
This command allows users of the AddressBook application to add optional remarks to people in their date book and edit it if required.
...
Let's start by creating a new `RemarkCommand` class in the `src/main/java/seedu/date/logic/command` directory.
...
[source, java]
----
package seedu.date.logic.commands;

import seedu.date.model.Model;

/**
 * Changes the remark of an existing person in the date book.
 */
...
[source, java]
----
import static seedu.date.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.date.logic.parser` package.
...
Create a new `Remark` in `seedu.date.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.date.ui.PersonCard`].
...
Not doing so will cause AddressBook to default to an empty date book!
