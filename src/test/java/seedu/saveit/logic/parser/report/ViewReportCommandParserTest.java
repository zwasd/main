...
public class ViewReportCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + START_DATE_DESC_MRT + END_DATE_DESC_MRT
                + VALID_GRAPH_BAR_DESC_CAPS + VALID_ORGANISATION_MONTH_DESC, new ViewReportCommand(expectedReport));

        // multiple graph type - last graph type accepted
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + START_DATE_DESC_MRT + END_DATE_DESC_MRT
                + VALID_GRAPH_PIE_DESC_CAPS + VALID_GRAPH_BAR_DESC_CAPS
                + VALID_ORGANISATION_MONTH_DESC, new ViewReportCommand(expectedReport));
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        assertParseFailure(parser, START_DATE_DESC_MRT + END_DATE_DESC_MRT
                        + VALID_GRAPH_BAR_CAPS + VALID_ORGANISATION_MONTH_DESC,
                expectedMessage);

        // missing organisation prefix
        assertParseFailure(parser, START_DATE_DESC_MRT + END_DATE_DESC_MRT
                        + VALID_GRAPH_BAR_DESC_CAPS + VALID_ORGANISATION_MONTH,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_START_DATE_MRT + VALID_START_DATE_MRT
                        + VALID_GRAPH_BAR_CAPS + VALID_ORGANISATION_MONTH,
                expectedMessage);
    ...
    public void parse_invalidValue_failure() {
        ...
        // invalid graph type
        assertParseFailure(parser, START_DATE_DESC_MRT + END_DATE_DESC_MRT
                        + INVALID_GRAPH_DESC + VALID_ORGANISATION_MONTH_DESC,
